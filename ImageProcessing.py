        ...
        for wv1 in wv_group1:
            for wv2 in wv_group2:
                ...
                if not tuple(wvdiff) in self.wvdiff_otfs.keys():
                    ...
                    # wrappers.wrapped_ifftn(psf_phase_shifted)[int(len(x)/2), int(len(y)/2), int(len(z)/2)]
        end = time.time()
    ...
    def Dj(self, q_grid, method="Fourier"):
        ...
        for m in range(len(self.illumination.waves)):
            ...
            if method=="Interpolation":
                d_j += np.abs(a_m)**2 * np.abs(self.optical_system.get_otf(q_grid - k_m))**2
            else:
        ...
        for m1 in range(len(self.illumination.waves)):
            for m2 in range(len(self.illumination.waves)):
                ...
                if method == "Fourier":
                    ...
                    otf1 = self.optical_system.get_otf(q_grid - k_m1)
                    if otf1 == 0:
                        continue
                    otf2 = self.optical_system.get_otf(q_grid - k_m2)
                    if otf2 == 0:
                        continue
