...
# plt.show()
otf = otf[2::2, 2::2]
optical_system = System4f3D(alpha = alpha, refractive_index_sample=nmedium, refractive_index_medium=nmedium)
...
stack = stack[:, 3:7, :, N//2+1:-N//2-1, N//2+1:-N//2-1]
...
stack = stack * mask[np.newaxis, np.newaxis, ..., np.newaxis]
print(stack.shape)
plt.imshow(stack[0, 3, 0, ...].T, cmap='gray', origin='lower')
plt.show()
plt.imshow(np.log1p(10 ** 8 * np.abs(wrapped_fftn(stack[0, 3, 0, ...]))).T, cmap='gray', origin='lower')
plt.show()
...
illumination = IlluminationPlaneWaves2D.init_from_3D(
    illumination3d, dimensions=(1, 1), force = True
)
...
illumination2d = pattern_estimator.estimate_illumination_parameters(
    stack, 
...
IlluminationPatternEstimator3D.illumination_3d_from_illuminaton_2d_and_total_wavelength(
    illumination3d, illumination2d, 642 / 680
)
