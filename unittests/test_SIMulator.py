...
class TestReconstruction(unittest.TestCase):
    ...
    def test_FDR(self):
        ...
        # image = ShapesGenerator.generate_sphere_slices(psf_size, N, r = 0.1, N = 100, I=10**4)
        R = (X**2 + Y**2 + Z**2)**0.5
        ...
        def update1(val):
            ...
            im1 = ax1.imshow(image_widefield[:, :, int(val)], vmin=0, vmax=np.amax(image_widefield))
            im2 = ax2.imshow(filtered_image[:, :, int(val)], vmin=0, vmax=np.amax(filtered_image))
            im3 = ax3.imshow(expected_image[:, :, int(val)], vmin=0, vmax=np.amax(expected_image))
            # im4 = ax4.imshow(image_filtered[:, :, int(val)], vmin=0)
