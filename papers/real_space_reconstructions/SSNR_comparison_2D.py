...
optical_system.compute_psf_and_otf((psf_size, N), high_NA=True, vectorial=True)
...
for kernel_size in [1, 5, 9]:
        ...
        ax2.set_xlabel(r"$f_r[LCF]$")
        ax1.set_ylabel(r"$1 + 10^4 SSNR_{ra}$")
        ax2.set_ylabel(r"$f_r[LCF]$")
        ...
        ax1.plot(two_NA_fx[fx >= 0], (1 + multiplier *ssnr_finite_3waves_ra), color='red', label="Conventional")
        ax1.plot(two_NA_fy[fy >= 0], (1 + multiplier *ssnr_finite_s_polarized_ra), color='green', label="Square")
        ax1.plot(two_NA_fy[fy >= 0], (1 + multiplier *ssnr_s_polarized_ra), color='green', linestyle='--')
        ax1.plot(two_NA_fx[fx >= 0], (1 + multiplier *ssnr_3waves_ra), color='red', linestyle='--')
        ...
        ax3.plot(two_NA_fx[fx >= 0], ssnr_finite_3waves_ra / ssnr_3waves_ra, color='red', label="Conventional")
        ax3.plot(two_NA_fx[fx >= 0], ssnr_finite_s_polarized_ra/ ssnr_s_polarized_ra, color='green', label="Square")
        ax3.set_aspect(1. / ax3.get_data_ratio())

        ax1.legend(fontsize=30, loc="upper right")
        ax3.legend(fontsize=30, loc="lower right")
        # ax2.legend(fontsize=15, loc="lower left")
        # plt.show()
        fig1.savefig(current_dir + f"/Figures/SSNR_finite_kernel_{kernel_size}.png", bbox_inches='tight')
        fig2.savefig(current_dir + f"/Figures/SSNR_ratio_{kernel_size}_ratio.png", bbox_inches='tight')
        # fig3.savefig(current_dir + f"/Figures/SSNR_ratio_ra{kernel_size}.png", bbox_inches='tight')
